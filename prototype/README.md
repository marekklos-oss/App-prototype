...
| `#/pojisteni-vozidlo-platby` | Detail pojištění — Vozidlo · Platby | `12531:51976` |
| `#/pojisteni-vozidlo-dokumenty` | Detail pojištění — Vozidlo · Dokumenty | `4408:27164` |
| `#/pojisteni-vozidlo-osoby` | Detail pojištění — Vozidlo · Osoby | `2292:40293` |
...
**Stav: VEHICLE je hotové celé** — `Shrnutí`, `Pojištění`, `Platby`, `Dokumenty`
i `Osoby`.
Zbývá PROPERTY (odložené, viz níž), PET a TRAVEL.
...
- **Teď se dělá jen VEHICLE.** PROPERTY (Dům), PET a TRAVEL později, ale
...
- Hash: `#/pojisteni-vozidlo` = smlouva, podstránka jde za pomlčku
  (`#/pojisteni-vozidlo-pojisteni`). PROPERTY jednou `#/pojisteni-dum-*`.
- Nav má `Shrnutí · Pojištění · Platby · Dokumenty · Osoby`, všech pět je hotových
  a proklikaných tam i zpátky. Nav je širší než displej, takže `render()`
  v `app.js` doskroluje aktivní pilulku do záběru — jinak by poslední taby
  po načtení nebyly vidět.
- **Tab „Moje vozidlo" se nedělá** — vyhozený z navu 12. 8. 2026 na zadání.
...
Ze Shrnutí vedou dál hlavičky karet: „Pojištění" → podstránka Pojištění,
„Dokumenty" → Dokumenty, „Osoby na smlouvě" → Osoby, „Profil vozidla" →
`#/profil-auta`.
...
`.scard__eyebrow` (drobnosti).

Podstránka Pojištění: `.covhead` + `.covhead__edit` (nadpis s tužkou), `.covnote`,
`.covlabel`, `.covcard` + `.covrow` (karta rizika s limitem), `.covexpand` +
`.expandbtn` (skládací odrážky — `data-expand`, sbalí se od 5. položky),
`.assistbox` (limetkový callout), `.pilltoggle--outline`, `.btn--danger-soft`.

Podstránka Platby: `.payrow` (+ `--due` pro nezaplacenou splátku), `.pilltoggle--lime`.
Podstránka Dokumenty: `.doclist` + `.doccard` + `.tag--soft`, `.archhead`
(rozbalovací hlavička archivu, `data-toggle-section`). `.covexpand` skládá
i `.doclist` — sbalí se od páté položky, stejně jako odrážky.
Podstránka Osoby: `.covcard--rows` (karta jen s `.trow` řádky). Jméno a tagy
jsou **nad** kartou, ne v ní.

**Osoby jsou na dvou místech** — karta „Osoby na smlouvě" na Shrnutí a celá
podstránka. Jména se musí měnit na obou. Údaje jsou smyšlené (`example.com`,
telefony 601/602 601 601), ve Figmě jsou jen `#### Kč` placeholdery.
Karta „Aktuální stav" je záměrně obsahově totožná s kartou Platby na Shrnutí —
když se mění částky, musí se to přepsat na obou místech.

Nové ikony ve sprite: `ic-eye`, `ic-check-circle`, `ic-trash`, `ic-chevron-down`,
`ic-check`.

**Platby mají ve Figmě 9 stavů** (`12531:51758`) — GREEN vše zaplaceno / s
přeplatkem, GRAY ukončená ve třech variantách, ORANGE nedoplatek / k zaplacení,
RED po splatnosti / + nedoplatek. Postavený je jen **GREEN „Vše zaplaceno"**,
protože odpovídá datům naší smlouvy. Zbytek se dá dodělat přes `.protobar`
přepínač stavů, jak to má `#/profil-auta`. Pozor: ta sekce ve Figmě míchá mobil
a web — mobilní framy poznáš podle šířky **375 px** a jména `Detail pojisteni -
platby`.
...
### Pravidlo: šipka = klik někam vede

Kolečko se šipkou u `POJIŠTĚNÍ` je **jediný signál, že je sekce klikací** —
takže se ukazuje právě tehdy, když za ní něco je. Nemusí to být detail smlouvy.

| Stav | Šipka | Klik vede |
|---|---|---|
...
- **Detail pojištění:** PROPERTY / PET / TRAVEL nejsou; Platby mají postavený
  jen stav „Vše zaplaceno" z devíti, co jsou ve Figmě
...
`#/profil-fiat` je **druhý profil vozidla** — do 13. 8. 2026 vedly obě kartičky
(Volvo i Fiat) na `#/profil-auta`, což byl profil Volva. Teď má každé auto svůj.

Fiat je záměrně **prázdné vozidlo**: pojištění ve stavu `Neuvedeno` (karta
z Figmy `24548:59120`, „Mám pojištění jinde" otevírá sheet `pojisteni-jinde`),
olej i pneu `Neuvedeno`, STK do 28. 8. 2026. Nemá `.protobar` ani `.vgroup`
varianty — stavový přepínač je pomůcka pro Volvo, Fiat má jen jeden stav.

Segmentovka `Stav · Hodnota · Technické údaje` je na něm **mrtvá**. Vedla by na
`#/profil-stav` a `#/profil-technicke`, což jsou data Volva.

## Odchylky od Figmy (vědomé)

- Karta pojištění ve stavu `Neuvedeno` má ve Figmě v hlavičce šipku; v prototypu
  je bez ní podle pravidla „šipka = klik někam vede" (karta si nese vlastní
  tlačítka „Mám pojištění jinde" a „Spočítat")
- Opravené překlepy: „Direct pojišťovna" (Figma: pojišťna), „Volvo XC90" v
  Základních údajích (Figma: Vovlo), „Přenos síly" (Figma: Přenost), nabídka
  povinného ručení (bylo „Pojistíme si v nice sazbu, dlouhodobě odškodní vozidla"
  a 4 909 Kč — Figma má „Pojisti si u nás auto, dostaneš náhradní vozidlo."
  a 4 929 Kč)
