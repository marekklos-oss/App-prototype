...
function assertFileKey(fileKey: string): void {
  if (!/^[A-Za-z0-9_-]{10,128}$/.test(fileKey)) {
    throw new Error("Invalid Figma file key.");
  }
}

function assertNodeIds(nodeIds: string[]): void {
...
async function figmaGet(path: string, params: Record<string, string | number | boolean | undefined> = {}) {
  ...
  if (!response.ok) {
    if (response.status === 403) throw new Error("Figma rejected the token. Check its expiry, file access, and file_content:read scope.");
    if (response.status === 404) throw new Error("Figma could not find this file or node. Check the link and file access.");
